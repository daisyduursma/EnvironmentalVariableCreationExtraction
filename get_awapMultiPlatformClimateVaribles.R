...
get_awap <- function(dates,climVar){
...
get_awap_one <- function(date,climVar){
  if(climVar == "tmax"){
  baseurl <- "http://www.bom.gov.au/web03/ncc/www/awap/temperature/maxave/daily/grid/0.05/history/nat/"
  }
  if(climVar == "tmin"){
    baseurl <- "http://www.bom.gov.au/web03/ncc/www/awap/temperature/minave/daily/grid/0.05/history/nat/"
  }
  
  if(climVar == "prec"){
    baseurl <- "http://www.bom.gov.au/web03/ncc/www/awap/rainfall/totals/daily/grid/0.05/history/nat_recal/"
  }
...
# example
a <- get_awap_one(as.Date("2015-06-01"),climVar="tmin")
plot(a)
b<- get_awap_one(as.Date("2015-06-01"),climVar="tmax")
plot(b)
c <- get_awap_one(as.Date("2015-06-01"),climVar="prec")
plot(c)
